        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredExercises.map((exercise) => (
              <div key={exercise.id} className="group relative rounded-2xl overflow-hidden bg-secondary cursor-pointer" onClick={() => openExercise(exercise)}>
                <div className="relative aspect-[4/3] overflow-hidden">
                  ...
                  <span className="absolute top-3 right-3 px-3 py-1 bg-background/90 text-foreground text-xs font-medium rounded-full">{exercise.category}</span>
                </div>
        ...
        <div className="fixed inset-0 z-50 bg-foreground/95 flex items-center justify-center p-4 overflow-y-auto" onClick={closeModal}>
          ...
          <div className="max-w-3xl w-full bg-background rounded-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            {selectedExercise.video ? (
              <video autoPlay controls className="w-full aspect-video" src={selectedExercise.video} poster={selectedExercise.image} />
            ) : (
              <img src={selectedExercise.image} alt={selectedExercise.name} className="w-full aspect-video object-cover" />
            )}
            <div className="p-6">
              <span className="text-sm font-medium uppercase tracking-wider text-muted-foreground">{selectedExercise.category}</span>
              <h2 className="font-display text-3xl font-semibold mt-2 mb-3">{selectedExercise.name}</h2>
              <p className="text-sm font-medium mb-4">{t("workouts.target")}: {selectedExercise.target}</p>
