              ...
              >
                <div className="relative aspect-[4/3] overflow-hidden">
                  ...
                  <span className="absolute top-3 right-3 px-3 py-1 bg-background/90 text-foreground text-xs font-medium rounded-full">
                    {l(
                      exercisesData.categories.find(
                        (c) => c.en === exercise.category,
                      ) || exercise.category,
                    )}
                  </span>
                </div>
          ...
          >
            ...
            <div className="p-6">
              <span className="text-sm font-medium uppercase tracking-wider text-muted-foreground">
                {l(
                  exercisesData.categories.find(
                    (c) => c.en === selectedExercise.category,
                  ) || selectedExercise.category,
                )}
              </span>
              <h2 className="font-display text-3xl font-semibold mt-2 mb-3">
                {l(selectedExercise.name)}
              </h2>
              <p className="text-sm font-medium mb-4">
                {t("workouts.target")}: {l(selectedExercise.target)}
              </p>
              <p className="text-muted-foreground">
                {l(selectedExercise.description)}
              </p>
            </div>
          </div>
        </div>
